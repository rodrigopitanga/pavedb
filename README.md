...
# 🛣️ PaveDB — Vector search you can understand & deploy within minutes

PaveDB is a single-process vector search engine for AI applications.
...
Built for developers shipping **RAG (Retrieval-Augmented Generation)** systems, PaveDB
...
- **Ingest files, not embeddings** — hand it a PDF, CSV, or TXT (more formats to come)
  and PaveDB chunks, embeds, and indexes it. No preprocessing pipeline to build.
...
[Flowlexi Container Registry](https://gitlab.com/flowlexi/pavedb/container_registry)
on GitLab (CUDA builds publish as `latest-gpu`, CPU-only as
`latest-cpu`).

```bash
docker pull registry.gitlab.com/flowlexi/pavedb/pavedb:latest-gpu
docker pull registry.gitlab.com/flowlexi/pavedb/pavedb:latest-cpu
```

Run the service by choosing the tag you need and mapping the API port locally:

```bash
docker run -d --name pavedb \
  -p 8086:8086 \
  registry.gitlab.com/flowlexi/pavedb/pavedb:latest-cpu
```
...
```bash
docker exec pavedb pavecli create-collection demo books
docker exec pavedb pavecli ingest demo books /app/demo/20k_leagues.txt \
  --docid=verne-20k --metadata='{"lang":"en"}'
docker exec pavedb pavecli search demo books "captain nemo" -k 3
```
...
# sample demo corpus
curl -LO https://raw.githubusercontent.com/rodrigopitanga/pavedb/main/demo/20k_leagues.txt

# set an admin key for the generated config
export PAVEDB_GLOBAL_KEY=super-sekret
...
By default, a non-dev runtime reads `~/pavedb/config.yml` if present, keeps
tenant sidecar loading disabled unless `auth.tenants_file` is configured, and
stores data in `~/pavedb/data`. You can override any of that with the
`PAVEDB_*` environment scheme or by pointing `PAVEDB_CONFIG` at an explicit
...
```bash
# create collection
curl -H "Authorization: Bearer $PAVEDB_GLOBAL_KEY" \
...
The Swagger UI is available at `http://localhost:8086/ui`.

Health and metrics endpoints are available at `/health` and `/health/metrics`.

Runtime options are also accepted via the `PAVEDB_*` environment variable scheme
(`PAVEDB_SERVER__PORT`, `PAVEDB_AUTH__MODE`, etc.), which precedes config files.

### 🔁 Live data updates

PaveDB supports live data refresh without restarting the server. Re-ingest the same
...
PaveDB writes human-readable logs to stderr and optionally emits
...
PaveDB is free software: you can use it, copy it, redistribute it and/or modify it
...
PaveDB is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY;
